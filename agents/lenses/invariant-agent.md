...
- **Break round-trips.** Make `deposit(X) → withdraw(all)` return more than X. Test with 1 lamport / 1 token base unit, `u64::MAX`, first/last deposit.
...
- **Reset timers via secondary call paths.** A function unconditionally updates a timestamp (`asset.timestamp = Clock::get()?.unix_timestamp`, `last_claim`) that an adversary uses to repeatedly reset a window (JIT, cooldown, lockup). Find every timestamp update not gated by an explicit branch.
...
- **Couple state-price reads across mutating paths.** Liquidation reads price and balance at different points in the same transaction; price moves between the reads (oracle refresh, swap CPI) and the liquidation pays the wrong amount.

...
**Seam 1 — execution × periphery.** A control path that's internally correct but whose downstream periphery call returns or behaves in a way that derails the trace. Example: a vault deposit follows a clean path, but it issues a transfer call for a Token-2022 mint that charges a transfer fee — the resulting balance differs from the expected amount, and subsequent code uses the pre-transfer value. The trace alone is "correct"; the periphery alone is "correct" (the token does what it says); the bug exists in the assumption the trace makes about what periphery returned.

**Seam 2 — periphery × first principles.** An external interaction that's safe in isolation but defeats the protocol's stated purpose when chained into the broader system. Example: protocol's purpose is "users always receive at least X." A correctly-written SPL transfer call to a Token-2022 mint with a transfer fee, or transfer hook violates that promise, even though the call site is technically correct. Find every periphery interaction whose downstream consequence undermines a stated guarantee.
...
- A flow that depends on periphery returning a specific shape (account layout, length, decimals) which non-standard programs/mints may not.
...
- A Token-2022 transfer hook (or other CPI) mid-flow that runs third-party code which issues its own CPIs and mutates accounts the caller reads after the call returns — resulting in inconsistent mid-flow state.
...
- Shared state written by program X and read as ground truth by program Y; the attacker bridges between programs to convert phantom state (pending shares, in-flight balances) into real claims.
...
- Cross-chain message handlers iterating over user-controlled lengths or combinatorial sets; legitimate users exceed block gas or compute units, bricking delivery.

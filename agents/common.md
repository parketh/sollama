...
These are the audit principles that a senior auditor follows. Pattern-matching catches the obvious bugs — your specialty file teaches that. The high-value bugs, the ones everyone else misses, come from HOW you reason about code, not from WHAT bugs you know.
...
When you find a bug in one program, **weaponize that pattern across every other program and instruction in the bundle.** Search by handler name AND by code pattern. Finding a missing signer or owner check in `ProgramA`'s `withdraw` handler means you check every other programs's `withdraw` account validation — missing a repeat instance is an audit failure.
...
Admin-only instructions doing admin things. Standard DeFi tradeoffs (rounding dust, first-depositor share inflation). Self-harm-only bugs. "Admin can rug" without a concrete mechanism.
...
path: caller → instruction → account/state change → impact
proof: concrete values/trace demonstrating the bug
fix: one-sentence suggestion
```

Findings are keyed by first row identifiers (program, instruction, class) for deduplication. 

Individual agents may add or substitute custom fields to the above, based on rules described in their `## Output fields` section.

...
1. Read all inputs. Upstream status guard: if `prepare-output.json` `status` is `"blocked"`, or `inspect-findings.md` metadata `Status` is `blocked`, or `static-analysis.md` metadata `Status` is `blocked`, or `candidate-findings.md` metadata `Status` is `blocked`, stop and report the upstream blockers; do not organize. The operator must resolve the upstream block and re-run that step first.
